...
(() => {
  ...
  function computeBoardBase() {
    return Math.min(window.innerWidth * 0.96, 650);
  }
  ...
  const previewImg = document.getElementById('preview') || null;
...
I love you, i love u more than u know, i love u more than I know how to say 🤍`;
  ...
  // Insert letter text into page
  document.addEventListener('DOMContentLoaded', () => {
    const p = document.getElementById('paperInner');
    if (p) p.textContent = LETTER_TEXT;
  });

  // ========== STATE ==========
  let BOARD = computeBoardBase();
  let CELL = BOARD / SIZE;
  let PAD = CELL * 0.35;
  let VB = CELL + 2 * PAD;

  let topSign = [], rightSign = [], bottomSign = [], leftSign = [];
  let slots = [];
  let trayOrder = [];
  let selected = null;
  ...
  // ========== RENDER SLOTS (visual drop targets) ==========
  function renderSlots() {
    if (!backdrop) return;
    // remove previous
    backdrop.querySelectorAll('.backdrop-slot').forEach(n => n.remove());
    for (let r = 0; r < SIZE; r++) {
      for (let c = 0; c < SIZE; c++) {
        const idx = r * SIZE + c;
        const slot = document.createElement('div');
        slot.className = 'backdrop-slot' + (slots[idx] === null ? ' empty' : '');
  ...
  function renderTray() {
    ...
    trayOrder.forEach((value, idx) => {
      ...
      svg.addEventListener('click', (e) => { e.stopPropagation(); onTrayClick(idx); });
  ...
  function updateStats() {
    ...
    if (correct === TOTAL) {
      ...
      // also open scrollWrap if exists
      ...
      if (paper) {
        ...
        requestAnimationFrame(() => { paper.style.height = targetHeight + 'px'; });
      }
    } else {
  ...
  // backdrop click (kept as fallback; slots stop propagation)
  if (backdrop) {
    backdrop.addEventListener('click', (e) => {
      ...
      if (selected === null) { flashWrong(r,c); return; }
      const pieceValue = trayOrder[selected];
      if (pieceValue === idx) {
        slots[idx] = pieceValue;
        trayOrder.splice(selected, 1);
        selected = null;
        renderTray(); renderBoard(); updateStats();
      } else {
        flashWrong(r,c);
        selected = null;
        renderTray();
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (previewBtn && previewImg) {
      let previewShown = false;
      previewBtn.addEventListener('click', () => {
        previewShown = !previewShown;
        previewImg.style.display = previewShown ? 'block' : 'none';
  ...
  // responsive
  window.addEventListener('resize', () => { recomputeSizes(); renderTray(); renderBoard(); }, { passive: true });
  window.addEventListener('orientationchange', () => { recomputeSizes(); renderTray(); renderBoard(); }, { passive: true });
